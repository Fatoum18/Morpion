...
def get_game_config_or_create (size,alignment): 
    grid_size = int('0'+size)
    alignment = int('0'+alignment)
    game_config = GameConfig.objects.filter(grid_size=grid_size,alignment=alignment).first()
    if not game_config:
        game_config = GameConfig(grid_size=grid_size,alignment=alignment)
        game_config.save()
